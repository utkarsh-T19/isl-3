...
import { commitAdminChanges } from '../services/githubCommitter';
...
function teamLabel(teamId) {
  if (Array.isArray(teamId)) return teamId.map((id) => teamMap[id]?.name ?? id).join(' + ');
...
const styles = {
  page: { padding: '20px 16px 40px', maxWidth: 800, margin: '0 auto' },
  glass: {
  ...
  heading:  { fontSize: 28, fontWeight: 800, letterSpacing: '-0.03em', marginBottom: 8 },
  subtext:  { color: 'rgba(250,250,245,0.65)', fontSize: 14, marginBottom: 24 },
  input: {
    width: '100%', padding: '12px 16px', borderRadius: 9999,
    border: '1px solid rgba(251,211,22,0.12)', background: 'rgba(255,255,255,0.08)',
    color: 'var(--text)', fontSize: 15, fontFamily: 'inherit', fontWeight: 600,
    outline: 'none', marginBottom: 16,
  },
  btnPrimary: {
    display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
    gap: 8, padding: '12px 24px', borderRadius: 9999, fontWeight: 700,
    fontSize: 15, fontFamily: 'inherit', cursor: 'pointer', border: 'none',
    background: '#FBD316', color: '#0E0C00',
    boxShadow: '0 4px 20px rgba(251,211,22,0.35)', width: '100%',
  },
  btnGhost: {
    display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
    gap: 8, padding: '10px 20px', borderRadius: 9999, fontWeight: 700,
    fontSize: 14, fontFamily: 'inherit', cursor: 'pointer',
    background: 'rgba(255,255,255,0.08)', color: 'var(--text)',
    border: '1px solid rgba(251,211,22,0.12)', textDecoration: 'none',
  },
  error:   { color: '#FF453A', fontSize: 13, fontWeight: 600, marginBottom: 12 },
  success: { color: '#32D74B', fontSize: 13, fontWeight: 600, marginTop: 8 },
};
...
function PassphraseGate({ onAuth }) {
  ...
  const handleSubmit = async (e) => {
    ...
    } catch { setError('Hashing failed — try again'); }
    ...
    <div style={styles.page}>
      <div style={{ ...styles.glass, maxWidth: 420, margin: '60px auto' }}>
        ...
        <form onSubmit={handleSubmit}>
          <input type="password" placeholder="Passphrase" value={value}
            onChange={(e) => setValue(e.target.value)} style={styles.input} autoFocus />
  ...
  return (
    <div style={styles.page}>
      <div style={{ ...styles.glass, maxWidth: 420, margin: '60px auto' }}>
        ...
        <form onSubmit={handleSubmit}>
          <input type="password" placeholder="ghp_xxxxxxxxxxxx" value={value}
            onChange={(e) => setValue(e.target.value)} style={styles.input} autoFocus />
...
function FixtureCard({ fixture, updateMatchWinner }) {
  ...
  const winnerBtnStyle = (val) => {
    const isActive = fixture.winner != null && JSON.stringify(fixture.winner) === JSON.stringify(val);
    return {
      padding: '8px 14px', borderRadius: 9999, fontWeight: 700, fontSize: 13,
      fontFamily: 'inherit', cursor: 'pointer',
      border: isActive ? '1px solid #FBD316' : '1px solid rgba(251,211,22,0.12)',
      background: isActive ? 'rgba(251,211,22,0.18)' : 'rgba(255,255,255,0.06)',
      color: isActive ? '#FBD316' : 'var(--text)',
  ...
  return (
    <div style={{
      background: 'rgba(255,255,255,0.045)',
      border: isCompleted ? '1px solid rgba(50,215,75,0.45)' : '1px solid rgba(251,211,22,0.12)',
      borderRadius: 16, padding: 20, marginBottom: 12,
    }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <span style={{ fontSize: 12, fontWeight: 800, textTransform: 'uppercase',
          letterSpacing: '0.04em', color: 'rgba(250,250,245,0.65)' }}>
        ...
        <span style={{
          ...
          background: isCompleted ? 'rgba(50,215,75,0.12)' : 'rgba(255,255,255,0.08)',
      ...
      <div style={{ fontSize: 17, fontWeight: 800, marginBottom: 12, lineHeight: 1.4 }}>
        {t1Label}{' '}
        <span style={{ color: 'rgba(250,250,245,0.35)', fontWeight: 600 }}>vs</span>
        {' '}{t2Label}
      </div>
      ...
      {isCompleted && fixture.winner && (
        <div style={{
          ...
          borderRadius: 12, background: 'rgba(50,215,75,0.08)',
          border: '1px solid rgba(50,215,75,0.2)', marginBottom: 12,
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        <button style={winnerBtnStyle(fixture.team1Id)} onClick={() => handleWinner(fixture.team1Id)}>{t1Label}</button>
        <button style={winnerBtnStyle(fixture.team2Id)} onClick={() => handleWinner(fixture.team2Id)}>{t2Label}</button>
        <button style={winnerBtnStyle('draw')}          onClick={() => handleWinner('draw')}>Draw</button>
      </div>

      {confirmed && <p style={styles.success}>✓ Winner set to {confirmed}</p>}
    </div>
  );
}
...
function DeployStatus({ status, error, onRetry }) {
  if (status === 'success') {
    return (
      <div style={{
        background: 'rgba(50,215,75,0.1)', border: '1px solid rgba(50,215,75,0.35)',
        borderRadius: 12, padding: '14px 20px', marginBottom: 16,
        display: 'flex', alignItems: 'center', gap: 10,
      }}>
        <span style={{ fontSize: 18 }}>🚀</span>
        <span style={{ color: '#32D74B', fontSize: 14, fontWeight: 600 }}>
          CSVs committed — app will refresh in 1–3 minutes after deploy
        </span>
      </div>
    );
  }
...
function AdminDashboard({ onLogout, onTokenInvalid }) {
  // useLeagueData() provides fixtures, leaderboard, updateMatchWinner, and reload
  const { fixtures, leaderboard, updateMatchWinner, reload } = useLeagueData();
  ...
  const handleCommit = useCallback(async () => {
    ...
    const result = await commitAdminChanges(pat, fixtures, leaderboard);

    if (result.success) {
      setDeployStatus('success');
      // Reload DataContext from fresh CSVs after a short delay
      // (give GitHub Pages a moment to deploy, but at least update
      // the local state immediately from the in-memory changes)
  ...
  }, [fixtures, leaderboard, reload, onTokenInvalid]);
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        ...
        <div>
          <h1 style={{ ...styles.heading, marginBottom: 4 }}>⚡ Admin Panel</h1>
          <p style={{ color: 'rgba(250,250,245,0.65)', fontSize: 14 }}>
            Select a date to manage fixtures
          </p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <Link to="/" style={styles.btnGhost}>← Home</Link>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        {selectedDate && (
          <button onClick={() => setSelectedDate(null)}
            style={{ ...styles.btnGhost, fontSize: 12, padding: '8px 14px' }}>
            Show All
          </button>
        )}
        {!selectedDate && (
      ...
      <DeployStatus status={deployStatus} error={commitError}
        onRetry={showRetry ? handleCommit : null} />

      {selectedFixtures.length === 0 ? (
        <div style={{ ...styles.glass, textAlign: 'center', padding: '48px 24px' }}>
          <div style={{ fontSize: 40, marginBottom: 12 }}>📅</div>
          <p style={{ fontSize: 16, fontWeight: 700, color: 'rgba(250,250,245,0.65)' }}>
        ...
        <>
          {selectedFixtures.map((f) => (
            <FixtureCard key={f.id} fixture={f} updateMatchWinner={updateMatchWinner} />
          ))}
          <button onClick={handleCommit} disabled={commitLoading}
            style={{ ...styles.btnPrimary, marginTop: 16, opacity: commitLoading ? 0.7 : 1 }}>
            {commitLoading ? '⏳ Committing CSVs…' : '🚀 Commit to GitHub'}
          </button>
        </>
      )}
    </div>
...
export default function AdminPanel() {
  ...
  const handleTokenInvalid = useCallback(() => setPhase('token'), []);

  if (phase === 'passphrase') return <PassphraseGate onAuth={() => setPhase('token')} />;
  if (phase === 'token')      return <TokenGate onToken={() => setPhase('dashboard')} />;
