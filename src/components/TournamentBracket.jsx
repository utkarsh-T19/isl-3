...
const getTeam = (id) => {
  if (!id) return null;
  if (Array.isArray(id)) {
    // combined team — return array of teams
...
const getQualStatus = (entry, pool, sportId) => {
  ...
  const myMax = entry.points + countRemaining(entry.teamId, sportId) * pts;
  ...
  const canBeatMe = pool.filter((e) => {
    ...
    const eMax = e.points + countRemaining(e.teamId, sportId) * pts;
...
const BracketSlot = ({ teamId, label, sublabel, isDerived, isTbd, isWinner }) => {
  ...
  return (
    <div style={{
      padding: '8px 12px',
      background: isWinner ? 'rgba(251,211,22,0.12)' : 'var(--surface-2)',
      border: `1px solid ${isWinner ? 'var(--yellow)' : teams[0] ? `${teams[0].color}44` : 'var(--border)'}`,
      borderRadius: '10px',
      minWidth: '110px',
      maxWidth: '140px',
...
const TournamentBracket = ({ sportId, poolA, poolB, bracket, isCombined }) => {
    ...
    <div>
      <div style={{ display: 'flex', gap: '0px', alignItems: 'center', overflowX: 'auto', paddingBottom: '8px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: `${GAP}px`, flexShrink: 0 }}>
          <BracketSlot teamId={sf1t1} sublabel="Pool A #1" isDerived={sf1IsDerived} />
          <div style={{ height: `${GAP}px` }} />
          <BracketSlot teamId={sf1t2} sublabel="Pool B #2" isDerived={sf1IsDerived} />

          <div style={{ height: `${GAP * 2}px` }} />

          <BracketSlot teamId={sf2t1} sublabel="Pool A #2" isDerived={sf2IsDerived} />
          <div style={{ height: `${GAP}px` }} />
          <BracketSlot teamId={sf2t2} sublabel="Pool B #1" isDerived={sf2IsDerived} />
        </div>
